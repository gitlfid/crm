...
// --- [AUTO-FIX] TAMBAHKAN KOLOM SIGNATURE JIKA BELUM ADA ---
$checkSig = $conn->query("SHOW COLUMNS FROM users LIKE 'signature_file'");
if ($checkSig->num_rows == 0) {
    $conn->query("ALTER TABLE users ADD COLUMN signature_file VARCHAR(255) DEFAULT NULL");
}
...
if (isset($_POST['update_profile'])) {
    ...
    $signature_file = null;
    $resOld = $conn->query("SELECT signature_file FROM users WHERE id = $user_id");
...
// Ambil Nama Divisi dengan Query Aman
if (!empty($user['division_id'])) {
    ...
    if ($checkDiv->num_rows > 0) {
        ...
        if ($resDiv && $resDiv->num_rows > 0) {
            ...
            if(isset($d['division_name'])) { $division_name = $d['division_name']; } 
            elseif(isset($d['name'])) { $division_name = $d['name']; } 
            elseif(isset($d['title'])) { $division_name = $d['title']; }
        }
    }
}
?>
