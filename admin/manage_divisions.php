...
<div class="p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto space-y-6 animate-fade-in-up">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
        <div>
            ...
            <p class="text-slate-500 dark:text-slate-400 mt-2 font-medium">Kelola daftar divisi / departemen perusahaan untuk pengelompokan pengguna.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
        <div class="bg-white dark:bg-[#24303F] rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 flex items-center gap-5 transition-transform hover:-translate-y-1 group">
            ...
            <div>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Total Divisi</p>
                <h4 class="text-3xl font-black text-slate-800 dark:text-white leading-none"><?= number_format($total_divs) ?></h4>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#24303F] rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-colors duration-300 flex flex-col min-h-[400px] relative">
        ...
        <div class="overflow-x-auto modern-scrollbar flex-grow pb-24">
            <table class="w-full text-left border-collapse table-fixed min-w-[600px]">
                <thead class="bg-slate-50/80 dark:bg-slate-800/50">
                    <tr>
                        <th class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-wider w-[15%] text-center">System ID</th>
                        <th class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-wider w-[50%]">Division Name</th>
                        <th class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-wider w-[35%] text-center">Division Code</th>
                    </tr>
                </thead>
                <tbody id="tableBody" class="divide-y divide-slate-100 dark:divide-slate-800/50">
                    <?php if($divs && $divs->num_rows > 0): ?>
                        ...
                        <tr class="data-row hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 align-middle text-center search-target">
                                <span class="font-mono font-bold text-slate-500 dark:text-slate-400">#<?= str_pad($row['id'], 3, '0', STR_PAD_LEFT) ?></span>
                            </td>

                            <td class="px-6 py-4 align-middle search-target">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-500 shrink-0 border border-indigo-100 dark:border-indigo-500/20">
                                        <i class="ph-bold ph-briefcase"></i>
                                    </div>
                                    <div class="font-bold text-slate-800 dark:text-slate-200 text-sm group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors break-words whitespace-normal">
                            ...
                            <td class="px-6 py-4 align-middle text-center search-target">
                                <span class="inline-flex px-3 py-1 rounded-lg bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300 font-black text-xs tracking-widest border border-slate-200 dark:border-slate-600 uppercase shadow-sm">
                            ...
                            <td colspan="3" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                                    <div class="w-20 h-20 rounded-3xl bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center mb-4 border border-slate-100 dark:border-slate-800 shadow-inner">
                                        <i class="ph-fill ph-buildings text-4xl text-slate-300 dark:text-slate-600"></i>
                                    </div>
                                    <h4 class="font-black text-slate-700 dark:text-slate-200 text-base mb-1">Tidak Ada Divisi</h4>
                                    <p class="text-sm font-medium">Belum ada departemen/divisi yang ditambahkan.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
    ...
    <div class="relative bg-white dark:bg-[#24303F] rounded-3xl shadow-2xl w-full max-w-lg transform scale-95 opacity-0 transition-all duration-300 modal-box flex flex-col overflow-hidden">
        <form method="POST">
            <div class="px-6 py-5 border-b border-indigo-500/20 bg-indigo-600 text-white flex justify-between items-center">
                <h3 class="text-base font-black flex items-center gap-2"><i class="ph-bold ph-buildings text-xl"></i> Add New Division</h3>
            ...
            <div class="p-6 space-y-5">
                <div>
                    <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-2">Division Name <span class="text-rose-500">*</span></label>
                    <div class="relative group">
            ...
            <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-end gap-3 shrink-0">
                <button type="button" onclick="closeModal('addDivModal')" class="px-5 py-2.5 rounded-xl font-bold text-slate-600 bg-white hover:bg-slate-100 border border-slate-200 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-600 transition-colors text-sm shadow-sm">Batal</button>
                <button type="submit" name="add_div" class="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-500/30 flex items-center justify-center gap-2 active:scale-95">
