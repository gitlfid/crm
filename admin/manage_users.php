...
include '../config/functions.php'; // Pindahkan ke atas agar fungsi email terbaca
...
// =========================================
// 2. BACKEND LOGIC (CRUD) - DIJALANKAN SEBELUM HTML
// =========================================
...
if (isset($_POST['add_user'])) {
    ...
    // [FIX] Bersihkan spasi di email agar fungsi mail berjalan lancar
    ...
    if (empty($msg)) {
        ...
        if($cek->num_rows > 0) {
            ...
            if ($conn->query($sql)) {
                // [FIX] Panggil fungsi email dengan variabel bersih ($email_clean)
                ...
                $msg = "<div class='alert alert-success'>User berhasil ditambahkan dan Email notifikasi telah dikirim!</div>";
            } else { 
                $msg = "<div class='alert alert-danger'>Error DB: " . $conn->error . "</div>"; 
            }
        }
    }
}
...
if (isset($_POST['edit_user'])) {
    ...
    $username = $conn->real_escape_string($_POST['username']);
    $email = $conn->real_escape_string(trim($_POST['email'])); // Trim juga di sini
    $phone = $conn->real_escape_string($_POST['phone']);
    $role = $conn->real_escape_string($_POST['role']);
    $division_id = !empty($_POST['division_id']) ? intval($_POST['division_id']) : "NULL";
    $job_title = $conn->real_escape_string($_POST['job_title']);
...
if (isset($_POST['reset_password'])) {
    ...
    if ($uData) {
        ...
        if ($conn->query("UPDATE users SET password='$new_hash', must_change_password=1 WHERE id=$id")) {
            // Email reset password biasanya sudah jalan, tapi kita pastikan trim juga
            if (function_exists('sendEmailNotification')) {
                sendEmailNotification(trim($uData['email']), "Reset Password", "Password baru Anda: $new_pass");
            }
            $msg = "<div class='alert alert-warning'>Password direset dan email dikirim.</div>";
        }
    }
}
...
// --- FETCH DATA (Setelah logika CRUD agar tabel terupdate) ---
