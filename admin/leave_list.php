...
// --- [BARU] LOGIC EXPORT EXCEL ---
if (isset($_POST['export_excel']) && ($is_manager || $is_hr || $is_admin)) {
    $filename = "Laporan_Cuti_" . date('Ymd_His') . ".xls";
    
    // Header agar browser mengenali sebagai file Excel
    header("Content-Type: application/vnd.ms-excel");
    header("Content-Disposition: attachment; filename=\"$filename\"");
    header("Pragma: no-cache");
    header("Expires: 0");
    
    // Filter Query Export (Sama seperti filter view dashboard)
    $whereEx = "WHERE 1=1";
    ...
    // Output Tabel HTML (Excel akan membacanya)
    echo "<table border='1'>";
    echo "<thead>
    ...
    echo "</tbody></table>";
    exit; // Stop eksekusi agar HTML halaman tidak ikut terdownload
}
