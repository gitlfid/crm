<?php
// Note: Variable $role_name, $user_division_id, dll sudah diload dari header.php
...
// =========================================================================
// LOGIKA MENU BUILDER (SAMA SEPERTI SEBELUMNYA)
// =========================================================================
if ($role_name === 'admin') {
    ...
    // LOGIKA USER STANDARD
    if (isset($conn) && !$conn->connect_error && $user_division_id > 0) {
        $allowed_ids = [];
        $resPerm = $conn->query("SELECT menu_id FROM division_permissions WHERE division_id = $user_division_id");
        if ($resPerm) {
            while($p = $resPerm->fetch_assoc()) $allowed_ids[] = intval($p['menu_id']);
        }
