...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Memindahkan Sidebar keluar dari <main> dan diletakkan bersebelahan dengan Panel Kanan
            if (sidebar && app && rightPanel && sidebar.parentNode !== app) {
                app.insertBefore(sidebar, rightPanel);
            }

            // 2. LOGIKA DARK MODE (Tailwind + Sinkronisasi CSS Bootstrap Lama)
            ...
            // Cek status dark mode saat pertama halaman dimuat
            ...
            // 4. LOGIKA TOGGLE SIDEBAR (Collapse Desktop & Slide Mobile)
...
<body class="bg-slate-50 text-slate-800 dark:bg-slate-900 dark:text-slate-200 font-sans antialiased overflow-hidden selection:bg-indigo-500 selection:text-white transition-colors duration-300">
    ...
    <div id="app" class="flex h-screen w-full overflow-hidden">
        ...
        <div id="right-panel" class="flex flex-col flex-1 w-full overflow-hidden relative">
                        ...
                        <div class="relative">
                            ...
                            <div id="profileDropdown" class="hidden absolute right-0 mt-4 flex w-64 flex-col rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-[#24303F] shadow-soft-lg z-50 overflow-hidden transition-all origin-top-right">
                                ...
                                <ul class="flex flex-col gap-1 px-4 py-2">
                                    <li>
                                        <a href="../admin/profile.php" class="flex items-center gap-3.5 rounded-lg px-2 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            <i class="ph ph-user text-xl"></i>
                                            Edit Profile
                                        </a>
                                    </li>
                                    <li>
                                        <a href="../admin/settings.php" class="flex items-center gap-3.5 rounded-lg px-2 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto relative bg-slate-50 dark:bg-slate-900 transition-colors duration-300">